  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Products Grid */}
        <div className={isMobile ? "" : "lg:w-3/4"}>
              ...
              >
                <div className="relative overflow-hidden">
                  ...
                  <button
                    ...
                    className={`absolute top-2 right-2 p-1 bg-white rounded-full shadow-md hover:bg-gray-50 ${isMobile ? 'w-6 h-6' : 'w-8 h-8'}`}
                  >
                    <Heart className={`text-gray-600 ${isMobile ? 'w-3 h-3' : 'w-4 h-4'}`} />
                  </button>
                  {!product.in_stock && (
                    <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                      <span className={`text-white font-semibold ${isMobile ? 'text-xs' : 'text-sm'}`}>Out of Stock</span>
                    </div>
                  )}
                </div>
                
                <CardContent className={isMobile ? "p-3" : "p-4"}>
                  <h3 className={`font-semibold mb-2 line-clamp-2 ${isMobile ? 'text-sm' : 'text-lg'}`}>{product.name}</h3>
                  {!isMobile && (
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <span className={`font-bold text-orange-600 ${isMobile ? 'text-lg' : 'text-xl'}`}>
                      ₹{product.price}
                    </span>
                    {product.rating && !isMobile && (
                      <div className="flex items-center">
                  ...
                  <Button
                    ...
                    className={`w-full bg-orange-600 hover:bg-orange-700 ${isMobile ? 'text-xs py-1 h-8' : ''}`}
                  >
                    <ShoppingCart className={`mr-1 ${isMobile ? 'w-3 h-3' : 'w-4 h-4'}`} />
